$(document).ready(function() {
    ...
    const template = (data) => {
        ...
        $element.find('.name').text(data.member);
    ...
    // 리뷰 수정 폼 열기
    $(document).off("click", ".reviewUpdateFormBtn").on("click", ".reviewUpdateFormBtn", function(e){ 
        e.preventDefault();
        $("#reviewForm .resetBtn").detach();
        let card = $(this).parents("div.review");
        $("#id").val(card.data("id"));
        $("#member").val(card.find(".card-header .name").html());
        $("#member").prop("readonly", true);
        let body = card.find(".card-text").html();
        body = body.replace(/(<br>|<br\/>|<br \/>)/g, '\r\n'); 
        $("#revContent").val(body);
        $("#reviewForm button[type='button']").attr("id", "reviewUpdateBtn");
        let resetButton = $("<button type='button' class='btn btn-secondary col-sm-1 resetBtn mx-2'>");
        resetButton.html("취소");
        $("#reviewForm .sendBtn").after(resetButton);
    });

    // 리뷰 수정 취소
    $(document).off("click", ".resetBtn").on("click", ".resetBtn", function(){ 
        dataReset();
        $("#member").prop("readonly", false);
        $("#reviewForm button[type='button']").attr("id", "reviewInsertBtn");
        $("#reviewForm button[type='button'].resetBtn").detach();
        $("#id").val(0);
    });

    // 리뷰 수정
    $(document).off("click", "#reviewUpdateBtn").on("click", "#reviewUpdateBtn", function(){
        let reviewId = $("#id").val();    
        if (!checkForm("#revContent","리뷰 내용을")) return;
        else {
            $.ajax({
                url:'/review/'+reviewId,
                method:'PUT',
                ...
                if(data != ""){
                    ...
                    dataReset();
                    console.log("Updated review data:", data); // 수정된 리뷰 데이터를 로그로 출력
                    $('#reviewList').find("div[data-id="+reviewId+"]").find(".card-text").html(data.revContent.replace(/(\r\n|\r|\n)/g, "<br />"));
                }
            }).fail(function(){
                alert("시스템에 문제가 있어 잠시 후 다시 진행해 주세요.");
            });
